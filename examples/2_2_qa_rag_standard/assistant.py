"""
Q&A Assistant with RAG - Multiple Documents Example
---------------------------------

This example demonstrates how to use the Cel.ai framework with multiple markdown documents
for a more comprehensive RAG implementation.
...
# Initialize the markdown provider with the docs directory
md_provider = MarkdownProvider(base_path=str(path))
...
# NOTE: just for testing purposes delete the collection
...
for file_path in markdown_files:
    ...
    # Process each file - this will handle chunking and metadata extraction
    processed_chunks = md_provider.process_file(
        file_path,
        chunk_size=1000,
        overlap=200
    )
    ...
    for chunk_data in processed_chunks:
        store.add_documents(
            ids=[f"{file_path}_chunk_{chunk_data['metadata']['chunk_index']}"],
